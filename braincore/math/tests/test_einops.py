...
def test_ellipsis_ops_numpy():
  ...
  for pattern in identity_patterns:
    assert np.array_equal(x, ein_rearrange(x, pattern)), pattern

  for pattern1, pattern2 in equivalent_rearrange_patterns:
    assert np.array_equal(ein_rearrange(x, pattern1), ein_rearrange(x, pattern2))

  for reduction in ["min", "max", "sum"]:
    for pattern1, pattern2 in equivalent_reduction_patterns:
      assert np.array_equal(ein_reduce(x, pattern1, reduction=reduction),
...
def test_reduction_imperatives():
  for reduction in REDUCTIONS:
    ...
    for pattern, axes_lengths, expected_result in test_cases:
      result = ein_reduce(bcm.from_numpy(input.copy()), pattern, reduction=reduction, **axes_lengths)
...
def test_reduction_imperatives_booleans():
  ...
  for axis in range(6):
    ...
    res_any = ein_reduce(bcm.from_numpy(x_np), pattern, reduction="any")
    res_all = ein_reduce(bcm.from_numpy(x_np), pattern, reduction="all")
