...
def _wrapped_fun(stateful_fun: StatefulFunction, states, return_states=True):
  ...
  def wrapped_branch(state_vals, *operands):
    ...
    for st, val in zip(states, state_vals):
      st.value = val
    out = stateful_fun.jaxpr_call_without_states(*operands)
    if return_states:
...
def cond(pred, true_fun: Callable, false_fun: Callable, *operands):
  ...
  _assign_states(all_states, state_vals)
  return out
...
def _wrapped_scan_fun(stateful_fun: StatefulFunction, states, pbar_runner=None):
  ...
  def wrapped_fun(new_carry, inputs):
    ...
    for st, val in zip(states, state_vals):
      st.value = val
    carry, out = stateful_fun.jaxpr_call_without_states(carry, inputs)
...
def scan(
  ...
  Args:
    ...
    unroll: optional positive int or bool specifying, in the underlying
      ...
      `unroll=False`).
    progress_bar: optional :class:`~.ProgressBar` instance to display the progress
...
@set_module_as('braincore.transform')
def for_loop(f, *xs, length: Optional[int] = None, reverse: bool = False, unroll: int | bool = 1,
  ...
  """
  _, ys = scan(f, None, xs, length=length, reverse=reverse, unroll=unroll, progress_bar=progress_bar)
  return ys
